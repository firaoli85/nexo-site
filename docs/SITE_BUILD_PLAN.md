...
| J2 | **Maryland 16+ passenger PSC tier has TWO different figure sets, both labelled VERIFIED** (`PR` §2e L311 without property damage; §4c L688 with $100,000 property damage, different sources) | **RESOLVED 2026-08-17 — see PROVIDER_REQUIREMENTS_RESEARCH J2 RESOLUTION.** Cell B was right: the tier DOES carry $100,000 property damage (COMAR 20.95.01.18B(3)). Cell A was a transcription elision and is superseded. | `PR` itself calls a wrong insurance minimum "actively harmful" — a provider relying on it could be uninsured carrying a Medicaid member. Two VERIFIED cells that disagree means neither is verified |
| J3 | **"$100,000 GL / 25-50-25" attributed to Maryland by `PR` §2e (with DO NOT PUBLISH) and to Virginia by `SEO` §3.3, which builds strongest-opening #1 on it** | **RESOLVED 2026-08-17 — see PROVIDER_REQUIREMENTS_RESEARCH J3 RESOLUTION.** Ruling: BOTH-GENERIC. The figure is one vendor's boilerplate published as ~10 states' minimum and is no state's requirement. UNBLOCKED ON JURISDICTION; the page must be RE-PREMISED, and the DO-NOT-PUBLISH order on the figure itself stands. | A page whose entire premise is correcting a jurisdictional error cannot itself get the jurisdiction wrong. Most likely the same vendor boilerplate circulates for both states — plausible, unproven by either document |
